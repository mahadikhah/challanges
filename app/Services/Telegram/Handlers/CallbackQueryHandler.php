 ...
 *
 * **The query is acknowledged before the work, not after.** Telegram shows a
 * loading state on the tapped button until `answerCallbackQuery` arrives, and a
 * `callback_query_id` expires; if the work throws, the update is retried, and by
 * then the id is stale and the acknowledgement would fail. Answering first means a
 * failing handler leaves a spinner that stops rather than one that never does — and
 * the acknowledgement itself is deliberately best-effort, because it is cosmetic
 * and must never be the reason an update is retried. Nothing user-facing rides on
 * it: a handler's reply is an ordinary message.
 *
 * **Chat type is not checked, unlike in `MessageHandler`.** A tap can arrive from a
 * button on a channel post — which is exactly how a public challenge will be joined
 * — and there is nothing to leak by allowing it, because the actor is resolved from
 * `callback_query.from` and every reply goes to that user's own private chat rather
 * than to the chat the button was in.
 */
