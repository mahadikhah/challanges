...
// `env()` answers `string|true|null` — a bare "true" in either of these
// variables would be nonsense for the package, so narrow to the string the
// config below actually means.
...
return [
    ...
    */
    ...
    |--------------------------------------------------------------------------
    |
    | Under the admin panel's own prefix, so the file logs sit beside Telescope
    | as one observability corner of the panel rather than a second standalone
    | tool with its own entry point to remember (and protect).
    |
    */
    ...
    */
    ...
    |--------------------------------------------------------------------------
    | The same chain the Inertia admin panel and Telescope already sit behind:
    | session auth, then the one is_admin check, then the package's own gate
    | (which resolves to the `viewLogViewer` gate defined in
    | AppServiceProvider — the same check as EnsureUserIsAdmin, so the two can
    ...
    */
    ...
    'api_stateful_domains' => is_string($apiStatefulDomains) ? explode(',', $apiStatefulDomains) : null,
